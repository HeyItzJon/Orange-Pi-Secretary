...
# See the comment above this line's own history: a lockfile regenerated
# locally (different platform than whatever last committed it) can block
# a fast-forward pull with "local changes would be overwritten by merge"
# even though nothing in it is worth keeping — both lockfiles are fully
# regenerated by npm install/build a few lines below regardless.
