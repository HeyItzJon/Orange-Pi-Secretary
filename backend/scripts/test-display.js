...
test("the display model carries the tasks page and a badge count", () => {
  ...
  assert.deepEqual(d.pages.map((p) => p.id), ["today", "week", "tasks", "money", "year", "weather", "commute", "wall", "system"]);
...
test("the menu reads Day, Week, Tasks, Portfolio, Stats, Weather, Commute, Wall, System — Jon's round-47 relabel, round-51's ESP wall control tab, round-53's System health tab, round-86's Weather tab, and the commute/ETA feature's dashboard page", () => {
  const d = buildDisplay({ items: [], config, now: NOW });
  assert.deepEqual(d.pages.map((p) => p.label), ["Day", "Week", "Tasks", "Portfolio", "Stats", "Weather", "Commute", "Wall", "System"]);
